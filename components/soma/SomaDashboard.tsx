...
function MessageActions({
  ...
  return (
    <div className={`flex items-center gap-0.5 mt-1.5 ${role === "user" ? "justify-end" : ""}`}>
      ...
      {onDownloadPdf && (
        <ActionIconButton onClick={onDownloadPdf} title="Download as PDF">
          <Download size={12} />
        </ActionIconButton>
      )}
...
const EXT_BY_LANG: Record<string, string> = {
  jsx: "jsx", js: "js", javascript: "js", tsx: "tsx", typescript: "ts",
  html: "html", css: "css", python: "py", py: "py", json: "json",
  bash: "sh", shell: "sh", sql: "sql", java: "java", go: "go",
  rust: "rs", php: "php", ruby: "rb", c: "c", cpp: "cpp",
}
...
function CodeBlock({ language, code }: { language: string; code: string }) {
  ...
  function handleDownload() {
    const ext = EXT_BY_LANG[language.toLowerCase()] ?? "txt"
    const blob = new Blob([code], { type: "text/plain" })
    const url = URL.createObjectURL(blob)
    const a = document.createElement("a")
    a.href = url
    a.download = `soma-code.${ext}`
    a.click()
...
async function downloadContentAsPdf(content: string, filename = "soma-response.pdf") {
...
export default function SomaDashboard({
  ...
  // ── Persistence ───────────────────────────────────────────────────────────
  // Every message — user or assistant — is saved with a `metadata` object so
  // reopening a conversation later reconstructs it exactly (image/audio/video/
  // code/tables/attachments), not just as flattened plain text.

  async function persist(
    userInput: string,
    assistantContent: string,
    assistantMeta?: Record<string, any> | null,
      ...
      try {
        const res = await fetch("/api/ai/conversations", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: "user", content: userInput, metadata: userMeta ?? null },
              { role: "assistant", content: assistantContent, metadata: assistantMeta ?? null },
            ],
          }),
        })
  ...
  // Used by "Regenerate" — appends only a new assistant message to an
  // already-existing conversation, without re-inserting the user turn.
  async function persistAssistantOnly(content: string, meta?: Record<string, any> | null) {
    ...
    try {
      await fetch("/api/ai/conversations/messages", {
        ...
        body: JSON.stringify({
          conversationId,
          messages: [{ role: "assistant", content, metadata: meta ?? null }],
        }),
      })
    } catch (e) {
      console.error(e)
  ...
  const runTurn = useCallback(
    async (
      ...
      historyForModel: DashboardMessage[],
      ...
      const savePair = async (assistantContent: string, meta: Record<string, any> | null) => {
        if (isRegenerate) {
          await persistAssistantOnly(assistantContent, meta)
        } else {
          const userMeta = attachmentsForThisMessage.length ? { attachments: attachmentsForThisMessage } : null
          await persist(userInput, assistantContent, meta, userMeta)
        }
      }
      ...
      try {
        if (resolvedTool === "image") {
          ...
          if (data.success && data.imageUrl) {
            const meta = { type: "image", capability: "image", imageUrl: data.imageUrl }
          ...
          await savePair(content, { type: "text", capability: "search", sources: data.sources })
          ...
          await savePair(userInput, { type: "audio", capability: "audio" })
          ...
          if (data.success && data.imageUrl) {
            ...
            await savePair(userInput, { type: "video", capability: "video", imageUrl: data.imageUrl })
          } else {
          ...
          const capability: Capability = isCode ? "code" : "general"
          setMessages((prev) => [
            ...prev,
            { role: "assistant", content, type: "text", capability, provider: data.provider, model: data.model, timestamp: Date.now() },
          ])
          await savePair(content, { type: "text", capability, provider: data.provider, model: data.model })
        }
      } catch (err) {
        console.error(err)
  ...
  // ── Message actions: copy / regenerate / edit / feedback / download ──────
  ...
  async function handleRegenerate(index: number) {
    ...
    await runTurn(userMsg.content, userMsg.attachments ?? [], historyForModel, capabilityUsed, true)
    setLoading(false)
    setCurrentlyGenerating("")
  }
                    ...
                    <div className="flex flex-col gap-2.5">
                      ...
                      <div className="pl-7">
                        ...
                        <MessageActions
                          ...
                          onDownloadPdf={
                            msg.type === "text"
                              ? () => downloadContentAsPdf(msg.content)
                              : undefined
                          }
                        />
                      </div>
